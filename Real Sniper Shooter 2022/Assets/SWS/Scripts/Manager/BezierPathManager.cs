...
namespace SWS
{
    ...
    public class BezierPathManager : PathManager
    {
        ...
        void OnDrawGizmos()
        {
            ...
            Gizmos.color = color1;
            Gizmos.DrawWireCube(bPoints[0].wp.position, size);
            Gizmos.DrawWireCube(bPoints[bPoints.Count - 1].wp.position, size);

            //assign line and waypoints color
            Gizmos.color = color2;
            for (int i = 1; i < bPoints.Count - 1; i++)
                Gizmos.DrawWireSphere(bPoints[i].wp.position, radius);

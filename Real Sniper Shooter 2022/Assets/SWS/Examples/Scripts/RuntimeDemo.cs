...
public class RuntimeDemo : MonoBehaviour
{
    ...
    void DrawExample4()
    {
        ...
        if (example4.moveRef.tween != null && !example4.moveRef.tween.isPaused
    ...
    void DrawExample6()
    {
        ...
        if (!example6.done && GUI.Button(new Rect(30, 130, 100, 20), "Add Message"))
        {
            //get messages reference of the movement script
            Messages messages = example6.moveRef.messages;
            //get message slot at the second waypoint
            MessageOptions opt = messages.GetMessageOption(1);

            //insert message to call our method with an argument
            opt.message.Add("ActivateForTime");
            opt.type.Add(MessageOptions.ValueType.Object);
            opt.obj.Add(example6.target);
            //fill remaining lists and set up message

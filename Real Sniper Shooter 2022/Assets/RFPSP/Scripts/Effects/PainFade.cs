...
public class PainFade : MonoBehaviour {
	...
	public void FadeIn ( Color color, Texture2D fadeTexture, float fadeLength ){
		...
		painFadeObj.AddComponent<TextMesh>();
		painFadeObj.transform.position = new Vector3 (0.5f, 0.5f, 1000);
	//	painFadeObj.GetComponent<TextMesh>().texture = fadeTexture;
	...
	IEnumerator DoFade ( float fadeLength, bool destroyTexture, float alpha ){
		...
		Color tempColor = GetComponent<TextMesh>().color; 
   		tempColor.a = 0.0f;//store the color's alpha amount
    	GetComponent<TextMesh>().color = tempColor;//set the guiTexture's color to the value of our temporary color var
		...
		while (time < fadeLength){
			...
			GetComponent<TextMesh>().color = tempColor;
			yield return null;
		}

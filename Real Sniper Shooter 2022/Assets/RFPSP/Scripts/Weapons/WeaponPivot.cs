...
//				Cursor.visible = false;
			...
			if (myTransform) {
				if (!float.IsNaN (pivot.position.x) && !float.IsNaN (pivot.position.y) && !float.IsNaN (pivot.position.z) && !float.IsNaN (x)) {
					myTransform.RotateAround (pivot.position, myTransform.up, x);
				}
				if (!float.IsNaN (pivot.position.x) && !float.IsNaN (pivot.position.y) && !float.IsNaN (pivot.position.z) && !float.IsNaN (y)) {

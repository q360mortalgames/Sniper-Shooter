...
public class Ironsights : MonoBehaviour {
	...
	void Update (){
		...
		if(Time.timeScale > 0.0f && Time.deltaTime > 0.0f && Time.smoothDeltaTime > 0.0f&&gunObj){//allow pausing by setting timescale to 0
...
//			}
			...
			&& WeaponBehaviorComponent.reloadLastStartTime + WeaponBehaviorComponent.reloadLastTime < Time.time){
				...
				if(!dzAiming){
                    ...
                    nextFov = WeaponBehaviorComponent.zoomFOV-((1-GameManager.instance.ZoomSlider.value)*(WeaponBehaviorComponent.zoomFOV-WeaponBehaviorComponent.MinzoomFOV));

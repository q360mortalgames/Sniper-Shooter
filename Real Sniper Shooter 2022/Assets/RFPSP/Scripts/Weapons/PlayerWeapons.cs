...
public class PlayerWeapons : MonoBehaviour 
{
	...
	public GameObject[] PrimaryWeapons,SecWeapons,MeleeWeapons;
	...
	void Start (){
       ...
       // weaponOrder[0] = PrimaryWeapons[PlayerPrefs.GetInt("SelWeapon1") - 1];
		Debug.Log("SelWepon"+PlayerPrefs.GetInt ("SelWeapon1"));
        weaponOrder [1] = PrimaryWeapons [PlayerPrefs.GetInt ("SelWeapon1") - 1];
        //weaponOrder [2] = SecWeapons [PlayerPrefs.GetInt ("SelWeapon2") - 1];
        //weaponOrder [3] = MeleeWeapons [PlayerPrefs.GetInt ("SelWeapon3") - 1];
        ...
        WeaponBehavior BackupWeaponBehaviorComponent = weaponOrder[0].GetComponent<WeaponBehavior>();
	...
	void Update (){
		...
		&& !switching//only allow one weapon switch at once
			&& (!CurrentWeaponBehaviorComponent.shooting || (FPSPlayerComponent.hitPoints < 1.0f&&GameManager.instance.PlayerDead))//don't switch weapons if shooting
		...
		&& !FPSWalkerComponent.holdingObject){
			...
			if(!sprintSwitching){
				...
				if(FPSPlayerComponent.hitPoints < 1.0f && !deadDropped&&GameManager.instance.PlayerDead){
	...
	public void DropWeapon ( int weapon){
		...
		if(!deadDropped){
			if(totalWeapons == 0 && currentWeapon != 0 && currentWeapon != backupWeapon){
				if(weaponOrder[backupWeapon].GetComponent<WeaponBehavior>().haveWeapon && FPSPlayerComponent.hitPoints > 1.0f&&GameManager.instance.PlayerDead){
	...
	public IEnumerator SelectWeapon ( int index, bool isOffhandThrow = false, bool endOffhandThrow = false){
		...
		WeaponObjAnimationComponent = weaponOrder[1].GetComponent<Animation>();

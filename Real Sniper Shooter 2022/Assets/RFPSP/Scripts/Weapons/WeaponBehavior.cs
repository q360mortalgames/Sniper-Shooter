...
//			liquidMask = bulletMask & ~(1 << 18);
			liquidMask = (bulletMask & ~(1 << 18))&~(1<<22)&~(1<<2);
...
//	}
	...
	void Update (){
//		if (ammo <= 0 && bulletsLeft <= 0) 
//		{
//			
//		}
//		LayerMask mask = 1<<22;
		Vector3 so = playerObj.transform.position + playerObj.transform.up * 0.8f;
		float pd = Vector3.Distance (mainCamTransform.position, so);
		RaycastHit testhit;
		/*if (Physics.Raycast (mainCamTransform.position, mainCamTransform.forward, out testhit, range, bulletMask)) {

            if (testhit.collider.gameObject.layer == 13) {
				if (testhit.collider.GetComponent<LocationDamage> () && testhit.collider.GetComponent<LocationDamage> ().AIComponent.enabled&&!PlayerWeapons.instance.switching) {
					InputComponent.fireHold = true;
				} else {
					InputComponent.fireHold = false;
			...
			if (testhit.collider.tag == "AmmoBox") {
				testhit.collider.transform.GetComponent<RefillAmmo> ().Focusing ();
			}
		} else {
			InputComponent.fireHold = false;
		}
        */
		if (Time.timeScale > 0 && Time.deltaTime > 0) {//allow pausing by setting timescale to 0
			...
			*/
			//Ramesh
			if (meleeSwingDelay == 0 && !unarmed) {
				GameManager.instance.curBulletsLeft = bulletsLeft;
				GameManager.instance.curAmmo = ammo;
				//pass ammo amount to Gui object to be rendered on screen
//				AmmoText1.ammoGui = bulletsLeft;//main color
//				AmmoText1.ammoGui2 = ammo;
//				AmmoText2[1].ammoGui = bulletsLeft;//shadow background color
...
//				}
	...
	void Fire(){
		...
		|| (semiAuto && semiState))){
			...
			if (!PlayerWeapons.instance.SwitchExecuting && ammo <= 0) 
			{
    ...
    public Vector3 LasttargethitPoint;
    public GameObject lasthitObj;
    bool lasthit = false;
    //weapon or projectile damage and effects for collider that is hit //Hit Ramesh
    void HitObject ( RaycastHit hit, Vector3 directionArg, bool isSecondCast = false ){
		...
		case 13://hit object is an NPC
			...
			if(hit.collider.gameObject.GetComponent<LocationDamage>() && hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.enabled){
				...
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker
                    if (GameManager.instance.LastTargetTokill)
                    {
                        if (hit.collider.gameObject.GetComponent<LocationDamage>().isHead)
                        {
                            LasttargethitPoint = hit.point;
                            lasthitObj = hit.collider.gameObject;
                            //GameObject Dummybullet = Instantiate(Resources.Load("AS_Bullet"), (mainCamTransform.position + (mainCamTransform.forward * playerDist)), mainCamTransform.rotation) as GameObject;
                            // Dummybullet.GetComponent<Rigidbody>().AddForce(lookDirection * 50, ForceMode.Impulse);
                            //Dummybullet.GetComponent<Rigidbody>().AddForce(mainCamTransform.forward, ForceMode.Impulse);
                            //Dummybullet.transform.position = Vector3.MoveTowards(mainCamTransform.position, hit.collider.transform.position, Time.deltaTime * 50);
                            print("Aihitted2222222222");
                        }
                        if (hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.CharacterDamageComponent.hitPoints <= 0)
                        {
                            LasttargethitPoint = hit.point;
                            lasthitObj = hit.collider.gameObject;
                            GameObject Dummybullet = Instantiate(Resources.Load("AS_Bullet"), (mainCamTransform.position + (mainCamTransform.forward)), mainCamTransform.rotation) as GameObject;
	...
	IEnumerator Reload(){
     ...
     //               GameManager.instance.ZoomSlider.value = 0;

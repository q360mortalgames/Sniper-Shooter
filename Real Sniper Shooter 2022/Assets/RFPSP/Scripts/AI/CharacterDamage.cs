...
public class CharacterDamage : MonoBehaviour {
	...
	public void ApplyDamage ( float damage, Vector3 attackDir, Vector3 attackerPos, Transform attacker, bool isPlayer, bool isExplosion, Rigidbody hitBody = null, float bodyForce = 0f ){
		...
		if (hitPoints <= 0.0f)
        {
            ...
            {
                GameManager.instance.CivilianKilled();
            }
			Debug.Log ("EnemyKilled" + GameManager.instance.EnemiesKilled);
			...
			if(bodies.Length < 2){//if NPC is only using one capsule collider for collision, instantiate ragdoll, instead of activating existing body part rigidbodies
				if (!hasDieAnim) 
					...
					RagDollDie (hitBody, bodyForce,true);
			}else{
				if (!ragdollActive) {
					RagDollDie (hitBody, bodyForce,false);
	...
	void RagDollDie(Rigidbody hitBody, float bodyForce,bool anim) {
	print ("RagDollDie");
		if (dieSound){
		...
		//GetComponent<HumanController>().Die();
...
//		}
		...
		else 
		{
            ...
            if (!AIComponent.useMecanim)
            {
                AIComponent.AnimationComponent.wrapMode = WrapMode.Once;
                AIComponent.AnimationComponent.CrossFade("die", 0);
            }
            else
            {
                AIComponent.AnimatorComponent.SetBool("Attack", false);
                AIComponent.AnimatorComponent.SetBool("Walk", false);
                AIComponent.AnimatorComponent.SetBool("Run", false);
                AIComponent.AnimatorComponent.SetTrigger("Die");

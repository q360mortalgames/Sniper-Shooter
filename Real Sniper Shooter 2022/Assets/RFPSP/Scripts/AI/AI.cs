...
//	[HideInInspector]
	...
	void Start(){
		...
		NPCRegistryComponent.RegisterIndicator();
	...
	void OnEnable(){
		...
		//objectWithAnims.localPosition = new Vector3 (0, 0, 0);
		timeout = 0.0f;
...
//				return;
	...
	void CanSeeTarget(){
		...
		if((TargetAIComponent && !TargetAIComponent.enabled) || (target && !target.gameObject.activeInHierarchy)){//Ramesh
			target = null;
...
//			if(AnimationComponent)
//
//			AnimationComponent.CrossFade("shoot", 0.1f);
//		}else{
...
//		}
		//AnimatorComponent.SetBool("Attacking", false);// Ashwini
	}
	...
	public void StartShooting()
	{
		if (!ShootNow) 
		{
			StartCoroutine (StartShootingNow ());
			ShootNow = true;
		}
	}
	IEnumerator StartShootingNow()
	{
		playerObj = Camera.main.transform.GetComponent<CameraControl> ().playerObj;
...
//					return false;
			...
			if (targetVisible) {
				...
				if ((distance < shootRange && angle < shootAngle)||ShootNow) {
					if (attackFinished) {

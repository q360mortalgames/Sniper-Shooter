...
//		startingPos = myTransform.position;
	...
	void FixedUpdate(){
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if(FPSPlayerComponent.hitPoints > 1.0f&&!GameManager.instance.PlayerDead){
			...
			if (grounded) {
				...
				if(Time.timeSinceLevelLoad > 0.5f){
					if(prone || crouched || (FPSPlayerComponent.hitPoints < 1.0f&&GameManager.instance.PlayerDead)){//also lower to crouch position if player dies
						if(crouched && !prone){
							if(FPSPlayerComponent.hitPoints < 1.0f&&GameManager.instance.PlayerDead){
						...
						}else if(prone ||( FPSPlayerComponent.hitPoints < 1.0f&&GameManager.instance.PlayerDead)){
							midPos = proneCamHeight;
							capsule.height = crouchingCapsuleHeight / 1.15f;
						}
					}else{

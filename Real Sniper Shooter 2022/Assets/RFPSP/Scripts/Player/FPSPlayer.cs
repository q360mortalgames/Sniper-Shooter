...
public class FPSPlayer : MonoBehaviour {
	...
	[HideInInspector]
	public Texture CrosshairGuiTexture;
...
//		MainMenuComponent.enabled = false;
		...
		CrosshairGuiTexture = CrosshairGuiObjInstance.GetComponent<Texture>();
		...
		hitmarkerGuiTexture = hitmarkerGuiObjInstance.GetComponent<Texture>();
...
//		healthGuiText = HealthText.GetComponent<GUIText>();
		...
		if(usePlayerHunger){
			...
			HungerGUIText = HungerText.GetComponent<TextMesh>();
		}
		...
		if(usePlayerThirst){
			...
			ThirstGUIText = ThirstText.GetComponent<TextMesh>();	
		}
...
//		}
		...
		if (hitPoints < 1.0f){
//			SendMessage("Die");//use SendMessage() to allow other script components on this object to detect player death
			GameManager.instance.ShowSavePopup();
